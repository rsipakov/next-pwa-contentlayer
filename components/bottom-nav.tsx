...
const BottomNav = () => {
	...
	return (
		<div className='sm:hidden'>
			<nav className='pb-safe w-full bg-neutrals-100 border-t dark:bg-neutrals-900 dark:border-neutrals-700 fixed bottom-0'>
				<div className='mx-auto px-6 max-w-md h-16 flex items-center justify-around'>
					{links.map(({ href, label, icon }) => (
						<Link key={label} href={href}>
							...
							>
								{icon}
								<span className='text-xs text-neutrals-900 dark:text-neutrals-100'>
									{label}
								</span>
							</a>
						</Link>
